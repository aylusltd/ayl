...
function WBIHorizontalTicker(a) {
    ...
    this.start = function() {
        var c = new Date();
        this.scrollerID = "wbi_scroller_" + c.getSeconds().toString() + c.getMilliseconds().toString() + Math.floor(Math.random() * 101).toString();
        var b = '<div class="wbiscroller"><div class="scrollingtext" id="' + this.scrollerID + '"></div></div>';
        document.writeln(b);
        window.wbiGlobalScope[this.scrollerID.toString()] = this;
        var d = "#" + this.scrollerID;
        this.scroller = $(d);
        this.scroller.mouseenter(this.OnMouseOver);
        this.scroller.mouseleave(this.OnMouseOut);
        this.UpdateList();
        this.startTimer();
        var arr=Array.prototype.slice.call(document.querySelectorAll('.wbiscroller'));
        arr.forEach(function(el){
            el.parentNode.removeChild(el);
            document.querySelector('#tickers').appendChild(el);
        })
    };
    this.startTimer = function() {
        if (document.all) {
            setInterval('ieIntervalHandler("' + this.scrollerID + '","UpdateValues")', this.refreshData)
        } else {
    ...
    this.StartAnimationTimer = function() {
        if (document.all) {
            setInterval('ieIntervalHandler("' + this.scrollerID + '","AnimateLoop")', this.moveInterval)
        } else {
            this.timer = setInterval(function(b) {
                b.AnimateLoop()
    ...
    this.getSymbolDiv = function(b) {
        var e = b.n.replace(".", "_");
        e = e.replace("/", "_");
        var d = '<span class="WLSymbolItem" id="' + e + '_display"><a class="sym" target="_blank" href="https://www.google.com/finance?q=' + b.n + '">';
    ...
    this.UpdateList = function() {
        ...
        $.getJSON("https://widgets.freestockcharts.com/WidgetServer/DynamicLists.ashx?name=" + this.listName + "&senderID=" + this.scrollerID + "&take=20&cbust=" + b + "&jsoncallback=?", function(e, f) {
            if ((e != null)) {
    ...
    this.UpdateValues = function() {
        ...
        var c = new Date();
        var b = c.getHours().toString() + c.getMinutes().toString() + c.getSeconds().toString() + c.getMilliseconds().toString();
        $.getJSON("https://widgets.freestockcharts.com/Ajaxserver.aspx?service=DATA&senderID=" + this.scrollerID + "&sym=" + this.sub_symbols + "&cbust=" + b + "&jsoncallback=?", function(e, f) {
            if ((e != null)) {
